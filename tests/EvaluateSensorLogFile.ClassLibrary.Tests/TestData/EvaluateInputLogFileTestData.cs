...
namespace EvaluateSensorLogFile.ClassLibrary.Tests.TestData
{
    public class EvaluateInputLogFileTestData : IEnumerable<object[]>
    {
        ...
        public EvaluateInputLogFileTestData()
        {
            ...
            {
                Name = "temp-1",
                Readings = new List<ReadingModel<decimal>> { new ReadingModel<decimal> { Timestamp = DateTime.Parse("2007-04-05T22:00"), Value = 72.4m } }
            };

            HumidityModel humidityModel = new HumidityModel
            {
                Name = "hum-1",
                Readings = new List<ReadingModel<decimal>> { new ReadingModel<decimal> { Timestamp = DateTime.Parse("2007-04-05T22:04"), Value = 45.2m } }
            };

            MonoxideModel monoxideModel = new MonoxideModel
            {
                Name = "mon-1",
                Readings = new List<ReadingModel<int>> { new ReadingModel<int> { Timestamp = DateTime.Parse("2007-04-05T22:04"), Value = 5 } }
            };
