...
namespace EvaluateSensorLogFile.ClassLibrary
{
    ...
    public class LogFile : ILogFile
    {
        ...
        public string EvaluateLogFile(string logContentsStr)
        {
            ...
            string result = EvaluateSensorLogRecords(sensorLogModel);
        ...
        private decimal ComputeStandardDeviation(List<ReadingModel<decimal>> readings)
        {
                ...
                switch (line[0])
                {
                    ...
                    case "thermometer":
                        ...
                        while (true)
                        {
                            ...
                            {
                                ReadingModel<decimal> readingModel = HandleDecimalReading(line[0], line[1]);
                    ...
                    case "monoxide":
                        ...
                        while (true)
                        {
                            ...
                            if (timeValueRegex.IsMatch(line[0]))
                            {
                                ReadingModel<int> readingModel = HandleIntegerReading(line[0], line[1]);
        ...
        /// <returns>A JSON string representing the sensor log quality control evaluation</returns>
        private string EvaluateSensorLogRecords(SensorLogModel sensorLogModel)
        {
        ...
        private ReadingModel<decimal> HandleDecimalReading(string timestampInputValue, string decimalInputValue)
        {
            ...
            ReadingModel<decimal> readingModel = new ReadingModel<decimal>
            {
                Timestamp = timestamp,
                Value = value
            };
        ...
        /// <returns>Integer sensor log record</returns>
        private ReadingModel<int> HandleIntegerReading(string timestampInputValue, string integerInputValue)
        {
            if (string.IsNullOrWhiteSpace(timestampInputValue))
            {
                throw new ArgumentException(Messages.NullWhitespaceString, nameof(timestampInputValue));
            ...
            ReadingModel<int> readingModel = new ReadingModel<int>
            {
                Timestamp = timestamp,
                Value = value
            };
